...
<div class="container mx-auto px-4">
    ...
    <div class="flex flex-col md:flex-row items-center mb-8">
        <div class="w-full md:w-1/3 mb-4 md:mb-0">
            <div class="w-full rounded-lg" style="max-width: 300px; max-height: 300px; position: relative; overflow: hidden; display: flex; justify-content: center; align-items: center;">
                <img src="<?php echo e($sheet->sheet_image_path ? asset($sheet->sheet_image_path) : asset('img/profile.png')); ?>" 
                     alt="Imagem da Ficha" 
        ...
        <div class="w-full md:w-2/3 md:ml-6">
            ...
            <p class="text-lg text-gray-700 dark:text-gray-300 mb-6" 
               style="min-height: 1.5em; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                <?php echo e($sheet->sheet_description); ?>
    ...
    <form action="<?php echo e(route('rpg.attributes.store', ['rpg_hash' => $rpg->hash])); ?>" method="POST" id="attributesForm">
        <?php echo csrf_field(); ?>
        
        <?php $__currentLoopData = ['PC', 'PL', 'PM', 'PB']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $type): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <div id="<?php echo e(strtolower($type)); ?>" class="tab-pane hidden">
            <h3 class="text-2xl text-white mb-4">Atributos <?php echo e($type); ?></h3>
            <div class="bg-gray-700 p-6 rounded-lg shadow-lg">
                <table class="min-w-full table-auto border-collapse" id="<?php echo e(strtolower($type)); ?>Table">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                    <?php $__currentLoopData = $attributes->where('attributes_type', $type); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $attribute): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr>
                            <td class="px-4 py-2 text-white"><?php echo e($loop->iteration); ?></td>
                            <td class="px-4 py-2">
                                <input type="text" name="attributes[<?php echo e($attribute->id); ?>][name]" value="<?php echo e($attribute->attributes_name); ?>" class="w-full p-2 bg-gray-700 text-black rounded-md">
                            </td>
                            <td class="px-4 py-2">
                                <input type="number" name="attributes[<?php echo e($attribute->id); ?>][price]" value="<?php echo e($attribute->attributes_price); ?>" class="w-full p-2 bg-gray-700 text-black rounded-md">
                            </td>
                            <td class="px-4 py-2 text-center">
                                <?php
                                    $pointsAvailableForType = $pointsAvailable[$attribute->attributes_type] ?? 0;
                                ?>
                                <span class="text-white"><?php echo e($pointsAvailableForType); ?> pontos</span>
                            </td>
                            <td class="px-4 py-2 text-center">
                            ...
                            <input type="hidden" name="attributes[<?php echo e($attribute->id); ?>][id]" value="<?php echo e($attribute->id); ?>">
                            <input type="hidden" name="attributes[<?php echo e($attribute->id); ?>][type]" value="<?php echo e($type); ?>">
                        </tr>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </tbody>
                </table>
                <button type="button" class="mt-4 bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-md" id="add<?php echo e($type); ?>Row">Adicionar Atributo</button>
            </div>
        </div>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\trall\OneDrive\Documentos\TCC\WEB\QualiaTTRPG\resources\views/sheets/show.blade.php ENDPATH**/ ?>